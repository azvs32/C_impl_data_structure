...
void *new_with_log(size_t size, const char *file, const char *function,
  ...
  if (!ptr) {
    ...
    log_message(LOG_LEVEL_DEBUG, file, function, line, "地址 %p 分配 %u 字节 ",
                ptr, size);
  }
  return ptr;
}
...
void delete_with_log(void *ptr, const char *file, const char *function,
  ...
  if (ptr) {
    free(ptr);
    log_message(LOG_LEVEL_DEBUG, file, function, line, "地址 %p 内存释放成功",
                ptr);
  } else {
...
void *realloc_with_log(void *ptr, size_t size, const char *file,
  ...
  if (!new_ptr) {
    ...
    log_message(LOG_LEVEL_DEBUG, file, function, line,
                "地址 %p 重新分配 %u 字节 ", ptr, size);
  }
  return new_ptr;
}
